...
class MetadataClient:
    ...
    def get_file_metadata(self, nome_arquivo: str) -> Optional[dict]:
        ...
        try:
            ...
            if response.sucesso:
                return {
                    'nome_arquivo': response.metadata.nome_arquivo,
                    'tamanho_total': response.metadata.tamanho_total,
                    'total_chunks': response.metadata.total_chunks,
                    'checksum_arquivo': response.metadata.checksum_arquivo,
                    'timestamp_criacao': response.metadata.timestamp_criacao,
                    'timestamp_modificacao': response.metadata.timestamp_modificacao,
                    'no_primario': response.metadata.no_primario,
                    'nos_replicas': list(response.metadata.nos_replicas),
                    'esta_completo': response.metadata.esta_completo
                }
            else:
    ...
    def get_chunk_locations(self, nome_arquivo: str) -> list:
            ...
            if response.sucesso:
                ...
                for chunk in response.chunks:
                    chunks.append({
                        ...
                        'no_primario': chunk.no_primario,
                        'nos_replicas': list(chunk.nos_replicas),
                        'checksum': chunk.checksum,
                        'tamanho_chunk': chunk.tamanho_chunk,
                        'disponivel': chunk.disponivel
                    })
                return chunks
            else:
    ...
    def register_node(self, node_id: str, endereco: str, porta: int, 
                     capacidade_storage: int) -> Optional[str]:
        ...
        try:
            ...
            if response.sucesso:
                return response.node_id_atribuido
            else:
    ...
    def get_available_nodes(self, apenas_ativos: bool = True) -> list:
    ...
    def get_node_for_operation(self, tipo_operacao: str, arquivo_nome: str, 
                              chunk_numero: int = -1) -> Optional[dict]:
    ...
    def process_heartbeat(self, node_id: str, status: str, chunks_armazenados: list) -> bool:
        """Envia heartbeat para o servidor de metadados"""
        try:
    ...
    def get_system_status(self, incluir_detalhes: bool = True, 
                         incluir_estatisticas: bool = True) -> Optional[dict]:
...
class HeartbeatSender:
    """Classe para enviar heartbeats periódicos ao servidor de metadados"""
    ...
    def start(self):
        ...
        if not self.running:
            ...
            self.thread.start()
            print(f"Heartbeat iniciado para nó {self.node_id} (intervalo: {self.interval}s)")
    ...
    def stop(self):
        ...
        if self.thread:
            self.thread.join()
        print(f"Heartbeat parado para nó {self.node_id}")
